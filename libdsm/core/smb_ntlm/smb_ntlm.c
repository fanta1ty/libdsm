...
void smb_ntlmssp_negotiate(const char *host, const char *domain,
    ...
    nego->host_offset =  32 + (uint32_t)strlen(domain);
...
#define __AUTH_APPEND(FIELD, value, size, cursor)                                       \
            ...
            auth-> FIELD ## _offset = 64 + (uint32_t)cursor;                            \
            cursor += size;
...
void smb_ntlmssp_response(uint64_t srv_challenge, uint64_t srv_ts,
    ...
    token->size = sizeof(smb_ntlmssp_auth) + strlen(host) * 2 + strlen(domain) * 2
                + strlen(user) * 2 + blob_size + 16 // Blob + HMAC
