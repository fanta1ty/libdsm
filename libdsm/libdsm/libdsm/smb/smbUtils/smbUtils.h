 ...
 *\param[in] src_len The length in byte of the input string (strlen will do the job for utf-8 or single byte encoding)
 *\param[out] dst Will be set to the newly allocated encoded string, you have to free() it after usage unless the function returns 0, in that case, it'll be set to NULL
 ...
 * It returns a newly allocated string that you need to free after usage
 *\param[in] src The UCS2-LE string to be converved to local encoding
 *\param[in] src_len The size in bytes of src
 *\param[out] dst will be set to the newly allocated and converted string. You have to free() it unless the function returns 0, in that case, it'll be set to NULL
